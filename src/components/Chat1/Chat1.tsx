...
import {
  ...
  updateStock,
  updateWeather,
} from "@/store/antropySlice";
import { addChat as AddAntropyChat } from "@/store/chatSlice";
...
import useAntropyAnswer from "@/hooks/useAntropyAnswer";
import useAntropyFork from "@/hooks/useAntropyFork";
import useAntropyFetch from "@/hooks/useAntropyFetch";
import useAntropyRetry from "@/hooks/useAntropyRetry";
...
const Chat1 = (props: Props) => {
  const { id } = props;
  const dispatch = useDispatch();

  const { chatThread, isFetching } = useAntropyFetch(id);
  const { handleFork } = useAntropyFork(id);
  const { handleRetry } = useAntropyRetry();
  ...
  const { handleAnswer, handleRewrite, handleCancel } = useAntropyAnswer({
    threadId: id,
    chatThread,
    setError,
    setErrorFunction,
  ...
  const handleSend = (text: string) => {
    if (text.trim() !== "") {
      ...
      console.log("Answer", newChat.answer);
      dispatch(addChat({ threadId: id, chat: newChat }));
      dispatch(AddAntropyChat({ threadId: id, chat: newChat }));
    }
  };
  ...
  return (
    <div className="flex flex-col items-center p-4">
      <div className="mb-4">
        <Image
          src="https://res.cloudinary.com/apideck/image/upload/w_196,f_auto/v1689100675/icons/anthropic-claude.png"
          alt="ChatGPT Logo"
      ...
      <div className={styles.container}>
        {chatThread?.chats.map((chat, index) => (
          <div
            ref={
              index === chatThread.chats.length - 1 ? chatContainerRef : null
            }
            key={index}
