...
import { addChatThread, selectChatThread } from "@/store/antropySlice";
import { RootState } from "@/store/store";
import { selectUserDetailsState } from "@/store/authSlice";
import { useCallback } from "react";

const useAntropyFork = (threadId: string) => {
  ...
  const handleFork = useCallback(async () => {
    ...
    try {
      ...
      router.push(`/antropic/${newChatThreadId}`);
    } catch (error) {
